...
public class ForestRound extends Round {
  private int i = 0;
  ...
  public ForestRound(int id) {
    super(id, "Forest", RoundType.FOREST);
    this.ran = new SecureRandom();
    setDisplayname("&2&lForest");
  }
  ...
  public void random(int loop) {
    int c = randomBlock();
    ForestEnum b = ForestEnum.getById(c);
  ...
  private int randomBlock() {
    return this.ran.nextInt(ForestEnum.getTotal());
  }
    ...
    floor.drawFloor(new BlockChange() {
          public void onBlock(Block block) {
            if (block.getType().equals(Material.WOOL)) {
              ForestEnum b = ForestEnum.getById(ForestRound.this.randomBlock());
    ...
    floor.drawFloor(new BlockChange() {
          public void onBlock(Block block) {
            if (block.getType().equals(Material.AIR))
              return; 
            ForestEnum b = ForestEnum.getById(ForestRound.this.i);
            if (!block.getType().equals(b.getType())) {
              block.setType(Material.AIR);
              ForestRound.this.blocks.add(block);
            } else if (block.getType().equals(b.getType()) && block.getData() != b.getData()) {
              block.setType(Material.AIR);
              ForestRound.this.blocks.add(block);
            } 
          }
