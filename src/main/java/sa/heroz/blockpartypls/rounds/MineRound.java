...
public class MineRound extends Round {
  private int i = 0;
  ...
  public MineRound(int id) {
    super(id, "Mine", RoundType.MINE);
    this.ran = new SecureRandom();
    setDisplayname("&7&lMine");
  }
  ...
  private int randomBlock() {
    return this.ran.nextInt(MineEnum.getTotal());
  }
    ...
    floor.drawFloor(new BlockChange() {
          public void onBlock(Block block) {
            if (block.getType().equals(Material.WOOL)) {
              MineEnum b = MineEnum.getById(MineRound.this.randomBlock());
    ...
    floor.drawFloor(new BlockChange() {
          public void onBlock(Block block) {
            if (block.getType().equals(Material.AIR))
              return; 
            MineEnum b = MineEnum.getById(MineRound.this.i);
            if (!block.getType().equals(b.getType())) {
              block.setType(Material.AIR);
              MineRound.this.blocks.add(block);
            } 
          }
        });
  }
