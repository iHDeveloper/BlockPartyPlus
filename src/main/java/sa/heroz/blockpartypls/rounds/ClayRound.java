...
public class ClayRound extends Round {
  private int i = 0;
  ...
  public ClayRound(int id) {
    super(id, "Clay", RoundType.CLAY);
    this.ran = new SecureRandom();
    setDisplayname("&6&lClay");
  }
  ...
  private int randomBlock() {
    return this.ran.nextInt(ClayEnum.getTotal());
  }
    ...
    floor.drawFloor(new BlockChange() {
          public void onBlock(Block block) {
            if (block.getType().equals(Material.WOOL)) {
              ClayEnum b = ClayEnum.getById(ClayRound.this.randomBlock());
    ...
    floor.drawFloor(new BlockChange() {
          public void onBlock(Block block) {
            if (block.getType().equals(Material.AIR))
              return; 
            ClayEnum b = ClayEnum.getById(ClayRound.this.i);
            if (block.getType().equals(b.getType()) && 
              block.getData() != (byte)b.getId()) {
              block.setType(Material.AIR);
              ClayRound.this.blocks.add(block);
            } 
          }
        });
  }
